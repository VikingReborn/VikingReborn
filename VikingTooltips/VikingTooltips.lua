-----------------------------------------------------------------------------------------------
-- Client Lua Script for VikingTooltips
...
function VikingTooltips:Init()
	Apollo.RegisterAddon(self)
end

function VikingTooltips:OnLoad()
    xmlTooltipDoc = XmlDoc.CreateFromFile("TooltipsForms.xml")
    self.xmlDoc = XmlDoc.CreateFromFile("TooltipsForms.xml")
    self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function VikingTooltips:OnDocumentReady()
...
function VikingTooltips:UnitTooltipGen(wndContainer, unitSource, strProp)
	...
	elseif strProp ~= "" then
		if not self.wndPropTooltip or not self.wndPropTooltip:IsValid() then
			self.wndPropTooltip = wndContainer:LoadTooltipForm("TooltipsForms.xml", "PropTooltip_Base", self)
		end
	...
	elseif karNPCDispositionUnitTypes[strUnitType] then
		...
		if unitSource:IsSettlerImprovement() then
			if unitSource:IsSettlerReward() then
				...
				local wndInfo = Apollo.LoadForm("TooltipsForms.xml", "UnitTooltip_Info", wndMiddleDataBlockContent, self)
		...
		if unitSource:IsFriendlyWarplotStructure() then
			...
			local wndCurrentTier = Apollo.LoadForm("TooltipsForms.xml", "UnitTooltip_Info", wndMiddleDataBlockContent, self)
			...
			if unitSource:CanUpgradeWarplotStructure() then
				...
				local wndCurrentCost = Apollo.LoadForm("TooltipsForms.xml", "UnitTooltip_Info", wndMiddleDataBlockContent, self)
	...
	elseif strUnitType == "PinataLoot" then
		...
		if tLoot then
			...
			elseif tLoot.eLootItemType == Unit.CodeEnumLootItemType.Cash then
				local wndCash = Apollo.LoadForm("TooltipsForms.xml", "UnitTooltip_Cash", wndMiddleDataBlockContent, self)
			...
			elseif tLoot.eLootItemType == Unit.CodeEnumLootItemType.VirtualItem then
				local wndLoot = Apollo.LoadForm("TooltipsForms.xml", "UnitTooltip_PinataLoot", wndMiddleDataBlockContent, self)
...
local function ItemTooltipHeaderHelper(wndParent, tItemInfo, itemSource, wndTooltip, tFlags)
	local wnd = Apollo.LoadForm(xmlTooltipDoc, "ItemTooltip_Header", wndParent)
...
local function ItemTooltipBasicStatsHelper(wndParent, tItemInfo, itemSource)
	...
	if tItemInfo.tLevelRequirement or tItemInfo.tDurability or bSpecialBind or tItemInfo.tUnique or (tItemInfo.tStack and tItemInfo.tStack.nMaxCount ~= 1) or itemSource:GetPowerLevel() then
		wndBox = Apollo.LoadForm(xmlTooltipDoc, "ItemTooltip_BasicStatsBox", wndParent)
	else
		return
	end
...
local function ItemTooltipSeparatorDiagonalHelper(wndParent)
	if #wndParent:GetChildren() > 1 then -- Separator is skipped if it's the very first element
		local wnd = Apollo.LoadForm(xmlTooltipDoc, "SeparatorDiagonal", wndParent)
	end
end
...
local function ItemTooltipSeparatorSmallLineHelper(wndParent)
	if #wndParent:GetChildren() > 1 then -- Separator is skipped if it's the very first element
		local wnd = Apollo.LoadForm(xmlTooltipDoc, "SeparatorSmallLine", wndParent)
	end
end
...
local function ItemTooltipBudgetPropHelper(wndParent, tblSortedBudgetPropList, bUseDiff, tItemInfo) -- E.G. Moxie, Grit, Brutality
	for idx, tCur in pairs(tblSortedBudgetPropList) do
		local wndBudgetFirst = Apollo.LoadForm(xmlTooltipDoc, "SimpleRowSmallML", wndParent)
		...
		if tCur.nDiff and bUseDiff and not tItemInfo.arRandomProperties then
			for idxD, tCurD in pairs(tCur.arDerived or {}) do
				local wndD = Apollo.LoadForm(xmlTooltipDoc, "SimpleRowSmallML", wndParent)
...
local function ItemTooltipImbuementHelper(wndParent, tItemInfo)
	...
	if tItemInfo.arImbuements then
		...
		local wndFirst = Apollo.LoadForm(xmlTooltipDoc, "SimpleRowSmallML", wndParent)
		...
		for idx, tCur in pairs(tItemInfo.arImbuements) do
			if tCur.bActive or tCur.bComplete then
				...
				if not tCur.bComplete and tCur.strObjective then
					local wndChallegeName = Apollo.LoadForm(xmlTooltipDoc, "SimpleRowSmallML", wndParent)
					...
					local wndObjective = Apollo.LoadForm(xmlTooltipDoc, "SimpleRowSmallML", wndParent)
				...
				if tCur.strSpecial then
					...
					local wndSpecial = Apollo.LoadForm(xmlTooltipDoc, "SimpleRowSmallML", wndParent)
				...
				if tCur.eProperty and tCur.nValue then
					...
					local wndProperty = Apollo.LoadForm(xmlTooltipDoc, "SimpleRowSmallML", wndParent)
...
local function ItemTooltipRuneHelper(wndParent, tItemInfo) -- If mousing over a rune item, not drawing runes on an item
	...
	if tItemInfo.tRuneInfo and tItemInfo.tRuneInfo.tSet then
		...
		for idx, tCur in pairs(tBonuses) do
			...
			local wndBonus = Apollo.LoadForm(xmlTooltipDoc, "SimpleRowSmallML", wndParent)
...
local function ItemTooltipFlavorHelper(wndParent, tItemInfo)
	...
	local wnd = Apollo.LoadForm(xmlTooltipDoc, "ItemTooltip_Flavor", wndParent)
...
local function ItemTooltipMonHelper(wndParent, tItemInfo, itemSource, tFlags) -- Sell, Buy, Buy Back, Repair
	...
	if (tFlags.bBuying or tFlags.nPrereqVendorId) and (itemSource ~= tFlags.itemCompare) then
		...
		if tPrereqInfo and tPrereqInfo.strText then
			...
			local wndPrereq = Apollo.LoadForm(xmlTooltipDoc, "SimpleRowSmallML", wndParent)
	...
	if tItemInfo.bSalvagable or tFlags.bBuying or tFlags.bBuyback or tItemInfo.tCost.arMonSell or bCanRepair then
		ItemTooltipSeparatorSmallLineHelper(wndParent)
		wndBox = Apollo.LoadForm(xmlTooltipDoc, "ItemTooltip_SalvageAndMoney", wndParent)
	end
	...
	if (tFlags.bBuying or tFlags.bBuyback) and tItemInfo.tCost.arMonBuy then
		for idx, tCur in pairs(tItemInfo.tCost.arMonBuy) do
			...
			if monPrice:GetAmount() ~= 0 then
				...
				local wnd = Apollo.LoadForm(xmlTooltipDoc, "SimpleRowSmallML", wndBox)
				xml = XmlDoc.new()
...
local function GenerateItemTooltipForm(luaCaller, wndParent, itemSource, tFlags, nCount)
	...
	if tFlags.bPermanent then
		wndTooltip = Apollo.LoadForm(xmlTooltipDoc, "ItemTooltip_Base", tFlags.wndParent, luaCaller)
	...
	VikingTooltips:ApplyVikingHudBackground(wndTooltip:FindChild("ItemTooltipBG"), "dd141122",  "darkgray")
	wndTooltip:FindChild("ItemTooltipBG"):SetAnchorOffsets(10, 26, -10, -10)
	if wndTooltipComp then
		VikingTooltips:ApplyVikingHudBackground(wndTooltipComp:FindChild("ItemTooltipBG"), "dd141122",  "darkgray")
		wndTooltipComp:FindChild("ItemTooltipBG"):SetAnchorOffsets(10, 26, -10, -10)
	end
...
local function GenerateSpellTooltipForm(luaCaller, wndParent, splSource, tFlags)
	...
	if tFlags and tFlags.bPermanent then
		wndTooltip = Apollo.LoadForm(xmlTooltipDoc, "SpellTooltip_Base", tFlags.wndParent, luaCaller)
	else
	...
	wndTooltip:FindChild("BGArt2"):Show(False)
...
local function GenerateBuffTooltipForm(luaCaller, wndParent, splSource, tFlags)
	...
	wndTooltip:SetAnchorOffsets(0, 0, 250, 32 + wndTooltip:FindChild("GeneralDescriptionString"):GetHeight())
...
function VikingTooltips:OnUpdateVitals()
	tVitals = Unit.GetVitalTable()
end
...
function VikingTooltips:HelperCalculateConValue(unitTarget)
...
function VikingTooltips:CreateCallNames()
...
function VikingTooltips:OnGenerateWorldObjectTooltip( wndHandler, wndControl, eToolTipType, unit, strPropName )
...
function VikingTooltips:OnMouseOverUnitChanged(unit)
	self:UnitTooltipGen(GameLib.GetWorldTooltipContainer(), unit, "")
end

---------------------------------------------------------------------------------------------------
-- Vikinghug Hud Redesign
---------------------------------------------------------------------------------------------------
function VikingTooltips:ApplyVikingHudBackground(wndHandler, bgcolor, txtcolor)
	if not wndHandler then return end
	-- Set Background Sprite & Color
	wndHandler:SetStyle("Picture", true)
	wndHandler:SetStyle("Border", false)
	wndHandler:SetSprite("CRB_NameplateSprites:sprNp_WhiteBarFill")
	wndHandler:SetBGColor(bgcolor)
	wndHandler:SetTextColor(txtcolor)
end

local ToolTipInstance = VikingTooltips:new()
